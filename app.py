...
# ----------------------
invoice_df = pd.read_csv("data/ca_invoice.csv")[["name", "grass_area",
                                                 "start_read_date",
                                                 "end_read_date", "volume",
                                                 "estimated_volume"]]
...
display_df = filtered_df.copy().rename(columns={
    "name": "Park Name",
...
chart_df['Invoice Period'] = chart_df['start_read_date'].astype(str) + " - " + chart_df['end_read_date'].astype(str)
...
if len(selected_parks) > 1:
    bar_chart = alt.Chart(chart_data).mark_bar().encode(
        x=alt.X('Invoice Period:N',
                title="Invoice Period",
                axis=alt.Axis(labelLimit=0)),  # Shorten long labels
        ...
        facet=alt.Facet('name:N', title="Park Name"),
        columns=2
    )
else:
    bar_chart = alt.Chart(chart_data).mark_bar().encode(
...
st.write("A positive value indicates that the actual water consumption is higher than the estimated need, "
...
# ----------------------
# Sidebar: Running Instructions (Optional)
# ----------------------
# The following section is optional. You can provide users with instructions on how to run the application.
# st.sidebar.markdown("---")
# st.sidebar.header("Running Instructions")
# st.sidebar.write(
#     """
#     **To run the application outside of an IDE:**
#
...
#        ```
#     4. (Optional) You can package it using PyInstaller or within a Docker container.
#     """
# )