...
df_ca_green_area = df_ca_green_area[pd.to_numeric(df_ca_green_area['SIRA NO'], errors='coerce').notna()]
...
df_ca_invoice['name'] = (
    df_ca_invoice['name']
    .str.replace('ÇANKAYA BELEDİYESİ', '', regex=False)
    .str.strip()
)
...
df_ca_invoice['start_read_date'] = pd.to_datetime(df_ca_invoice['start_read_date']).dt.date
...
df_ca_name_similarity = similarity.best_matches(
    list(df_ca_green_area["PARK ADI"].unique()),
    list(df_ca_invoice["name"].unique())
)

# 1) Filter the similarity DataFrame for high-confidence matches
score_threshold = 0.95
df_ca_name_similarity_filtered = df_ca_name_similarity[df_ca_name_similarity.score > score_threshold].copy()

# 2) Merge df_ca_green_area with the filtered similarity DataFrame (inner join)
df_ca_matched = (
    df_ca_green_area[["PARK ADI", "ÇİM ALAN"]]
    .merge(
...
df_ca_invoice["water_need_m3"] = df_ca_invoice.apply(partial_calculate_total_water, axis=1)
...
df_ca_invoice["water_need_total"] = df_ca_invoice["water_need_m3"] * df_ca_invoice["grass_area"]

df_ca_invoice = df_ca_invoice[[
    "subscription", "name", "water_need_total", "volume",
    "start_read_date", "end_read_date", "grass_area"
]].copy()
